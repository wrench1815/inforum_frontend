    ...
    async editForm() {
      if (this.loggedInUserRole == 'Admin') {
        const data = {
          ...
          queryId: this.oldAnswer.queryId,
          userId: this.oldAnswer.userId,
        }

        await this.$axios
          .$put(`/ForumAnswer/${this.oldAnswer.id}`, data)
          .then((res) => {
            let timerInterval

            this.$swal({
              title: 'Success!',
              html: 'Answer updated successfully. <br /> Redirecting in <b></b> Seconds.',
              type: 'success',
              icon: 'success',
              timer: 2000,
              ...
              this.$router.push(
                `/admin/forum/answers/preview/${this.oldAnswer.id}`
              )
            })
          })
          .catch((err) => {
            ...
            try {
              ...
              msg = 'Unable to update answer.<br/>Please Try Again Later.'
            }
  ...
  created() {
    ...
    answer
      ...
      .catch((err) => {
        ...
        this.$router.push(`/admin/forum/answers/${this.$route.params.id}`)
      })
  },
}
</script>
